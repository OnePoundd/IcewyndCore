...
public class WitherSpawn implements CommandExecutor, Listener {
	...
	public void ondamage(EntityDamageEvent event){
			...
			if (e.getCustomName().equals("§4§l§oWither King")) {
				if (plugin.getConfig().getBoolean(".WitherInvin") == true) {
					...
					if (plugin.getConfig().getInt(".WitherPhase") == 0) {
						...
						WitherSkeleton mob = (WitherSkeleton) Bukkit.getWorld("world").spawnEntity(WitherSpawn.add(-4, 0, 0),EntityType.WITHER_SKELETON);
						WitherSkeleton mob2 = (WitherSkeleton) Bukkit.getWorld("world").spawnEntity(WitherSpawn.add(4, 0, 4),EntityType.WITHER_SKELETON);
						WitherSkeleton mob3 = (WitherSkeleton) Bukkit.getWorld("world").spawnEntity(WitherSpawn.add(2, 0, -2),EntityType.WITHER_SKELETON);
		...
		public void ondeath(EntityDeathEvent event) {
			...
			if (((LivingEntity) e).getHealth() < 25) {
				if (plugin.getConfig().getInt(".WitherPhase") == 1) {
					plugin.getConfig().set(".WitherPhase", 2);
					plugin.saveConfig();
					Bukkit.broadcastMessage("25% Health, summoning Blaze Guards");
